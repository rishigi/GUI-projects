...
class ExerciseCorrectionApp:
    def __init__(self, root):
        ...
        for i, (text, command) in enumerate(zip(button_texts, button_commands)):
            ...
            Button(self.control_frame, text=text, width=20, height=2, font=("Helvetica", 10),
                   bg="#61AFEF", fg="white", command=command).grid(row=row, column=column,)

        self.pause_button = Button(self.control_frame, text="Pause", width=20, height=2, font=("Helvetica", 12),
                                   bg="#E5C07B", fg="white", command=self.toggle_pause)
        self.pause_button.grid(row=len(button_texts)//7 + 1, column=0, columnspan=2)

        Button(self.control_frame, text="Quit", width=20, height=2, font=("Helvetica", 12), bg="#E06C75", fg="white",
               command=self.quit_app).grid(row=len(button_texts)//7 + 1, column=5, columnspan=2)
